...
while true do
    ...
    if JBpressed then
        if player and not jbing then
            ...
            local dir = hrp.AssemblyLinearVelocity
            if dir.Y > 1 then
                hrp.AssemblyLinearVelocity = Vector3.new(dir.X,dir.Y * -2, dir.Z)
            else
                hrp.AssemblyLinearVelocity = Vector3.new(dir.X,dir.Y * 4, dir.Z)
            end
            wait(0.1)
            hrp.AssemblyLinearVelocity = Vector3.new(dir.X*2, 75 , dir.Z*2)
            JBTxt.Color = green
            wait(0.2)
            JBTxt.Color = white
        end
